...
export async function getMcAfeeMetrics() {
    ...
    `;
        db.all(query, [], (err, rows) => {
            ...
            resolve({
                filesScanned: rows.find((r) => r.VALUE_NAME === "files_scanned")?.VALUE ?? null,
                threatsDetected: rows.find((r) => r.VALUE_NAME === "threats_detected")?.VALUE ?? null,
                threatsResolved: rows.find((r) => r.VALUE_NAME === "threats_resolved")?.VALUE ?? null,
                quarantineCount: rows.find((r) => r.VALUE_NAME === "items_quarantined")?.VALUE ?? 0,
